...
object FancyCellStyle {
  val DEFAULT_CELL_STYLE_INDEX:Short = -1

  // Need a workbook to create a Cell Style. Dangerous to use implicit
  // So FactoryMethod needs to be in FancyWorkbook?
  //  def apply() : FancyCell = {
...
/**
  * Another mutable class  to implement the java CellStyle interface
  */
class FancyCellStyle extends CellStyle {
  ...
  override def setShrinkToFit(shrinkToFit : Boolean) : Unit = _shrinkToFit = shrinkToFit

  override def getShrinkToFit : Boolean = _shrinkToFit
}
