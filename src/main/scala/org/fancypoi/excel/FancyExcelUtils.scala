...
object FancyExcelUtils {
  ...
  def colAddrToIndex(col: String) = {
    col.toList.reverse.zipWithIndex.foldLeft(0) {
      case (n, (alphabet, index)) =>
        n + scala.math.pow(26.toDouble, index.toDouble).toInt * (
  ...
  def searchFont(workbook: FancyWorkbook, font: FancyFont) = {
    val indexes = (0 until workbook.getNumberOfFonts).toList.filterNot(_ == 4).map(_.toShort)
    indexes.find {
      index =>
  ...
  def searchStyle(workbook: FancyWorkbook, style: FancyCellStyle) = {
    (0 until workbook.workbook.getNumCellStyles).map(_.toShort).find {
      index =>
        val registered =
          workbook.workbook.getCellStyleAt(index)
        equalStyleWithoutFont(style, registered) &&
          equalFont(style.getFont, workbook.getFontAt(registered.getFontIndex))
    }.map(v ⇒ workbook.workbook.getCellStyleAt(v))
  }
  ...
  def equalRichTextStyring(str1: RichTextString, str2: RichTextString, w1: Workbook, w2: Workbook): Boolean = {
    (str1, str2) match {
      case (s1: HSSFRichTextString, s2: HSSFRichTextString) =>
        ...
        !s1FontIndexes.zip(s2FontIndexes).exists {
      ...
      //				!x1FontIndexes.zip(x2FontIndexes).find {case (f1, f2) => !FancyPOIUtil.equalFont(workbook.getFontAt(f1),
      // workbook.getFontAt(f2))}.isDefined
      case _ => false
    }
  }
